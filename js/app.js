document.addEventListener("DOMContentLoaded",()=>{
  const who=requireLogin();if(!who)return;const me=FRIENDS.find(f=>f.name===who);document.getElementById("who-am-i").textContent=who;
  const mode=Backend.init();document.getElementById("mode-note").textContent=mode==="cloud"?"Connected to shared FiveZone data.":"Local demo mode — Firebase is not connected.";
  ...
  document.getElementById("chat-form").addEventListener("submit",e=>{e.preventDefault();const input=document.getElementById("chat-input"),text=input.value.trim();if(!text)return;Backend.sendMessage(who,text);input.value="";});
